...
import { Plus, Search, QrCode, Edit, Trash2, Eye, X, Package } from 'lucide-react'
...
export default function PatrimonioPage() {
  ...
  async function loadAssets() {
    ...
    if (error) console.error('[Patrimônio] Erro ao carregar:', error)
    setAssets(data || [])
    setLoading(false)
  }
  ...
  async function handleSave() {
    ...
    if (editing) {
      ...
      await supabase.from('assets').update({ qr_code_url: `${appUrl}/asset/${data.id}` }).eq('id', data.id)
      toast.success('Ativo cadastrado!')
    }
    setShowModal(false); setEditing(null); resetForm(); loadAssets()
  }
            ...
            <tbody>
              {filtered.map(asset => (
                <tr key={asset.id}>
                  ...
                  <td className="hidden lg:table-cell"><span className={`badge ${asset.possui_manutencao ? 'badge-info' : 'badge-default'}`}>{asset.possui_manutencao ? 'Sim' : 'Não'}</span></td>
                  <td>
      ...
      {showDetail && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowDetail(null)}>
          <div className="modal animate-scale-in max-w-2xl">
            <div className="modal-header">
              <div>
                <h2 className="text-sm font-semibold text-gray-800">{showDetail.nome_item}</h2>
                <p className="text-xs text-gray-400">ID: {showDetail.id.slice(0, 8)}...</p>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => downloadQR(showDetail)} className="btn-secondary text-xs"><QrCode size={12} /> QR Code</button>
            ...
            <div className="modal-body">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Manutenção Preventiva</span>
                  <button onClick={() => setShowRuleModal(true)} className="btn-secondary text-xs py-1"><Plus size={11} /> Adicionar Regra</button>
                </div>
                {rules.length === 0 ? <p className="text-xs text-gray-400 py-2">Nenhuma regra cadastrada</p> : (
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Histórico</span>
